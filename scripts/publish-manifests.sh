...
#             (docker.io/pipestreamai/grparse, git.rokkon.com/...); each
...
# attestation manifest per platform. The docker binary comes from PATH, so a
# test can stand in a fake one (scripts/test-publish-manifests.sh).
set -euo pipefail
