...
RUN apt-get update && apt-get install -y --no-install-recommends \
    ca-certificates cmake g++ git make ninja-build pkg-config libopencv-dev \
    && rm -rf /var/lib/apt/lists/*
WORKDIR /src
COPY . .
RUN --mount=type=cache,target=/build cmake -S . -B /build -G Ninja -DCMAKE_BUILD_TYPE=Release \
 && cmake --build /build --target grparse-server --parallel 4 \
 && mkdir -p /out \
...
ENV GRPARSE_LISTEN_ADDRESS=0.0.0.0:50051 GRPARSE_MODELS_DIR=/models
RUN apt-get update && apt-get install -y --no-install-recommends libcudnn9-cuda-13 poppler-utils libopencv-core-dev libopencv-imgcodecs-dev libopencv-imgproc-dev \
    && rm -rf /var/lib/apt/lists/*
