...
The service listens on `localhost:50051` and implements `ai.pipestream.parse.v1.ParseService` from the local `parse.proto` contract. `ConvertSource` currently accepts one `FileSource` containing base64-encoded PDF, PNG, JPEG, or TIFF bytes. It renders TEXT, MARKDOWN, HTML, and JSON outputs from the merged document (an empty `to_formats` keeps the plain-text default alone) and returns `INVALID_ARGUMENT`, naming the offender, for populated options and output formats it does not implement (DOCTAGS, HTML_SPLIT_PAGE, YAML, VTT, DOCLANG).
