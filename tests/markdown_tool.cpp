// Development utility behind the Markdown validation harness
// (scripts/validate_markdown.py): loads a serialized
// ai.pipestream.document.v1.Document, binary by default or protobuf-JSON
// with --json, and writes the Markdown rendering to stdout or -o.
...
int usage(const char* program) {
  std::println(stderr,
               ...
               "  and writes its Markdown rendering",
               program);
  return 2;
}
...
int main(int argc, char** argv) {
  ...
  std::string rendered;
  try {
    rendered = grparse::render_markdown(document);
