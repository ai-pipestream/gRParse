...
void verify_table_serialization_and_its_degradations() {
  ...
  ChunkOptions pipes;
  pipes.use_markdown_tables = true;
...
void verify_picture_chunks_emit_markdown_image_placeholder() {
  docv1::Document bare = new_document();
  add_picture(&bare, {});
  ChunkOptions with_images;
  with_images.use_markdown_images = true;
  ...
  ChunkOptions custom;
  custom.use_markdown_images = true;
  custom.image_placeholder = "[img]";
  custom.image_placeholder_set = true;
...
void verify_raw_text_mirrors_text_when_requested() {
  ...
  ChunkOptions with_raw;
  with_raw.include_raw_text = true;
  const auto raw = chunk_hierarchical(document, {}, with_raw, "d.pdf");
