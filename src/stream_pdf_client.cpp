...
int main(int argc, char** argv) {
  ...
  pipestream::parse::v1::DocumentChunk final_chunk;
  ...
  while (stream->Read(&event)) {
    if (event.has_page()) {
      ...
      for (const auto& offset : event.page().text_offsets()) {
        if (offset.source() == pipestream::parse::v1::TEXT_SOURCE_DIGITAL_PDF) ++digital_items;
        if (offset.source() == pipestream::parse::v1::TEXT_SOURCE_OCR) ++ocr_items;
      }
